        ...
        <div class="mb-5 position-relative">
            <nav aria-label="breadcrumb" class="animate-in" style="--animation-order: 1">
                <ol class="breadcrumb mb-2" style="font-size: 0.75rem; letter-spacing: 0.5px; text-transform: uppercase;">
                    ...
                    <li class="breadcrumb-item active text-primary fw-bold" aria-current="page">My Submissions</li>
                </ol>
            </nav>
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 animate-in" style="--animation-order: 2">
                <div>
                    <h2 class="fw-800 mb-1" style="color:var(--text); letter-spacing: -0.5px;">My <span class="text-orange">Submissions</span></h2>
        ...
        <div class="tab-content pt-2">
            ...
            <div class="tab-pane fade show active" id="tabPending">
                <?php if (empty($pending)): ?>
                    <div class="scc-card p-5 text-center animate-in" style="--animation-order: 3">
                ...
                <?php else: ?>
                    <div class="scc-card scc-premium-shadow border-0 animate-in" style="--animation-order: 3">
                                ...
                                <tbody>
                                    <?php foreach ($pending as $sub): ?>
                                        <tr class="submission-item" data-search="<?= strtolower(htmlspecialchars($sub['course_code'] . ' ' . $sub['course_title'])) ?>">
                                            ...
                                            <td class="d-none d-md-table-cell small"><?= htmlspecialchars($sub['school_year'] ?? '—') ?></td>
                                            <td><?= format_syllabus_status($sub['status'], $sub['current_stage_role'] ?? null) ?></td>
                                            <td class="text-center">
                                                <a href="view_syllabus.php?file=<?= urlencode(basename($sub['file_path'])) ?>" target="_blank" class="text-primary hover-lift d-inline-block">
                                                    <i class="bi bi-file-earmark-pdf fs-5"></i>
                                                </a>
                                            </td>
                                            <td class="small text-muted"><?= date('M d, Y', strtotime($sub['submitted_at'])) ?></td>
                                            <td class="text-center">
                                                <a href="edit_syllabus.php?id=<?= $sub['id'] ?>" class="btn btn-sm btn-outline-warning rounded-pill px-3">
                                ...
                                <tbody>
                                    <?php foreach ($approved as $sub): ?>
                                        <tr class="submission-item" data-search="<?= strtolower(htmlspecialchars($sub['course_code'] . ' ' . $sub['course_title'])) ?>">
                                            ...
                                            <td class="text-center">
                                                <a href="view_syllabus.php?file=<?= urlencode(basename($sub['file_path'])) ?>" target="_blank" class="text-success hover-lift d-inline-block">
                                                    <i class="bi bi-file-earmark-pdf fs-5"></i>
                                                </a>
                                            </td>
                                            <td class="small text-muted"><?= date('M d, Y', strtotime($sub['submitted_at'])) ?></td>
                                ...
                                <tbody>
                                    <?php foreach ($rejected as $sub): ?>
                                        <tr class="submission-item" data-search="<?= strtolower(htmlspecialchars($sub['course_code'] . ' ' . $sub['course_title'])) ?>">
                                            ...
                                            <td class="text-center">
                                                <a href="view_syllabus.php?file=<?= urlencode(basename($sub['file_path'])) ?>" target="_blank" class="text-danger hover-lift d-inline-block">
                                                    <i class="bi bi-file-earmark-pdf fs-5"></i>
                                                </a>
                                            </td>
                                            <td class="text-center">
                                                <a href="edit_syllabus.php?id=<?= $sub['id'] ?>" class="btn btn-sm btn-outline-danger rounded-pill px-3">

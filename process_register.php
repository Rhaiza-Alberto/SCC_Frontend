...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validation
    if (empty($first_name) || empty($last_name) || empty($email) || empty($password) || empty($confirm_password) || empty($birthdate) || empty($sex) || empty($department_id)) {
    ...
    try {
        ...
        $stmt = $conn->prepare("
            INSERT INTO users 
                (first_name, middle_name, last_name, birthdate, sex, email, password, role_id, department_id, created_at, is_deleted, reset_requested)
            VALUES 
                (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), 0, 0)
        ");
        $stmt->execute([
        ...
        $dept_head = get_department_head($department_id);
        if ($dept_head) {
            notify_user(
                $dept_head['id'],
